...
export default function Edit() {
    const { student, genders, statuses } = usePage<{
        student: Student;
        genders: Array<{ key: string; value: string }>;
        statuses: Array<{ key: string; value: string }>;
    }>().props;

    const { data, setData, patch, processing, errors } = useForm<Required<EditStudentForm>>({
        name: student.name,
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <form onSubmit={submit} className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div className="flex justify-end md:col-span-2">
                        <Button disabled={processing}>Update Student</Button>
                    </div>
                </form>
            </div>
        </AppLayout>
